...
// ImageNoPadding returns the QR Code as an image.Image where the image
// is scaled up to eat the offset.
//
// size is both the width and height in pixels.
func (q *QRCode) ImageNoPadding(size int) image.Image {
	// Minimum pixels (both width and height) required.
	realSize := q.symbol.size
	...
	// Scale up ratio
	iSize := realSize * pixelsPerModule
	fSize := float64(size)

	rect := image.Rectangle{Min: image.Point{0, 0}, Max: image.Point{iSize, iSize}}
	...
	// img now contains an image which has an `iSize` image which we need to
	// scale up to `fSize`.
	return resize.Resize(uint(fSize), 0, img, resize.Lanczos3)
}
